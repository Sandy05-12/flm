...
def view_workload():
    if session.get('admin'):
        ...
        print(data)
        # Identify conflicts and non-conflicting leave periods
        ids_without_conflict = set()

        for i in range(len(data)):
            id_i, start_i, end_i = data[i][0], data[i][3], data[i][4]
            
            conflict_found = False
            
            for j in range(len(data)):
                if i != j and data[i][2] == data[j][2]:  # Avoid comparing the same entry
                    start_j, end_j = data[j][3], data[j][4]
            ...
            if not conflict_found:
                for j in range(len(data)):
                    if i != j and data[i][2] == data[j][2]:
                        ids_without_conflict.add((data[j][0], id_i,data[i][2]))  # Add pairs with comparison entry first
...
@app.route('/all_leave_details', methods=['GET'])
def all_leave_details():
    cursor = mydb.cursor(dictionary=True)
    
    # Fetch leave applications for all faculty members
    cursor.execute('''
    ...
    # Fetch leave allocation details for all faculty members
    cursor.execute('''
    ...
    for leave in leave_applications:
        ...
        if faculty_id not in leave_applications_by_faculty:
            leave_applications_by_faculty[faculty_id] = []
        leave_applications_by_faculty[faculty_id].append(leave)

    # Group leave allocations by faculty_id
    leave_allocations_by_faculty = {}
    for allocation in leave_allocations:
        faculty_id = allocation['faculty_id']
        if faculty_id not in leave_allocations_by_faculty:
            leave_allocations_by_faculty[faculty_id] = []
        leave_allocations_by_faculty[faculty_id].append(allocation)
